...
TEST(TestReleaseBug, bugfixTest) {  // NOLINT
  ...
  ASSERT_TRUE( example.init(H, g, A, lb, ub, lbA, ubA, nWSR, NULL) == REFER_NAMESPACE_QPOASES SUCCESSFUL_RETURN );
  ASSERT_TRUE( example.isSolved() == REFER_NAMESPACE_QPOASES BT_TRUE );
  example.getPrimalSolution(solution);

  printf( "\nxOpt = [ %e, %e ];\n\n", solution[0],solution[1] );

  for( REFER_NAMESPACE_QPOASES uint_t i=0; i<2; i++ )
    ASSERT_NEAR( solution[i],expectedFirst[i], 1e-10 );
}
